...
router.beforeEach((to, from, next) => {
  ...
  const isLogin = ()=>{//是否登录
    axios.post(common.api+'/wet/due/checkIsBind',{openId:store.state.loginInfo.openId})
        .then(function (res) {
  ...
  if (to.matched.some(record => record.meta.requireAuth)) {
    if(store.state.loginInfo.openId){
      ...
      if(code){ //根据code获取openId
        axios.post(common.api+"/wet/chat/getOpenId", {code:code}).then(function(res) {
